                                                            ...
                                                            InkWell(
                                                              onTap: () {
                                                                Get.to(const RestaurantListScreen(),
                                                                    arguments: {"vendorList": controller.newArrivalRestaurantList, "title": "New Arrival"})?.then((v) {
            ...
            Constant.selectedMapType == "osm"
                ? RepaintBoundary(
                    child: osmMap.OSMFlutter(
                        ...
                        osmOption: const osmMap.OSMOption(
                          userTrackingOption: osmMap.UserTrackingOption(
                            enableTracking: true,
                            unFollowUser: false,
                          ),
                          zoomOption: osmMap.ZoomOption(
                            initZoom: 14,
                            minZoomLevel: 2,
                            maxZoomLevel: 19,
                            stepZoom: 1.0,
                          ),
                          roadConfiguration: osmMap.RoadOption(
                            roadColor: Colors.yellowAccent,
                          ),
                        ),
                        onMapIsReady: (active) async {
                          if (active) {
                            controller.homeController.allNearestRestaurant.forEach(
                              (element) {
                                controller.mapOsmController
                                    .addMarker(osmMap.GeoPoint(latitude: element.latitude!, longitude: element.longitude!),
                                        markerIcon: osmMap.MarkerIcon(iconWidget: controller.departureOsmIcon),
                                        angle: pi / 3,
                                        iconAnchor: osmMap.IconAnchor(
                                          anchor: osmMap.Anchor.top,
                                        ))
                                    .then((v) {
                                  controller.osmMarkers['marker_${element.id}'] = osmMap.GeoPoint(latitude: element.latitude!, longitude: element.longitude!);
                                });
                              },
                            );
                          }
                            ...
                            Expanded(
                              child: PageView.builder(
                                ...
                                onPageChanged: (value) async {
                                  if (Constant.selectedMapType == "osm") {
                                    await controller.mapOsmController.moveTo(
                                        osmMap.GeoPoint(
                                          latitude: controller.homeController.allNearestRestaurant[value].latitude!,
                                          longitude: controller.homeController.allNearestRestaurant[value].longitude!,
                                        ),
                                        animate: true);
                                  } else {
